...
## "Today's prices" chart view

The chart offers an optional "Today's prices" toggle. It scales past snapshot values up to today's purchasing power using a fixed assumed inflation rate of 2.5% per year:

```
adjusted value = entered value x (1.025) ^ years since the snapshot
```

This is a viewing adjustment only. Your stored snapshots are unchanged, and 2.5% is an assumption, not a forecast. The toggle is meant to stop a gently rising nominal line from flattering you when most of the rise is inflation.
...
The calculator does not model taxes, fees, or investment returns, and it does not adjust your stored values for inflation (the optional "Today's prices" chart view applies a fixed 2.5% assumed rate for display only). It shows the values you enter. If you want to project future net worth, you would need to estimate those effects separately.
